...
public:
  ...
  Creature(string creatureParams) {
    ...
    // ====> DJH: -5% your code was trying to read initial calories from the ss
    // Feel while experimenting to find the source of the memory error. Most
    // of the time, these are related to unitialized vars (usually pointers)
    _initialCalories = _calories;
  }

  // return true if the Creature's position equals the passed position.
  bool positionEquals(int row, int col) { return _row == row && _col == col; }
  ...
  void move(const TurnInfo &turnInfo) {
    ...
    // ====> DJH: -3% This logic should be in the conditional above that tests
    // for acutal motion (changing _row or _col)
  ...
  void rest() {
    // ====> DJH: -5% Should rest (gain cals) unconditionally
    // should change the _resting state inside the recuperation condition
  ...
  void setTile(GameBoard *board) const {
    ...
    board->setTileAt(_row, _col, _glyph, Color::blue);
  }
...
private:
  ...
  static constexpr double kRestingRatio = 0.75;
...
public:
  ...
  void addCreaturesFromFile(string fileName) {
    ...
    while (getline(file, line)) {
      ...
      _creatures.push_back(myCreature);
    }
  ...
  int creatureCount() {
    ...
    // ====> DJH: Here it is!  No return. There should really be a compiler warning
    // about this!  Sheesh
  ...
  void clearTiles(GameBoard *board) {
    ...
    // one.
    for (int i = 0; i < creatureCount(); ++i) {
      _creatures.at(i)->clearTile(board);
    }
  }
  ...
  void takeTurns(const TurnInfo &turnInfo) {
    ...
    // one.
    for (int i = 0; i < creatureCount(); ++i) {
      Creature *creature = _creatures.at(i);
...
string MessageLine(CreatureList *creatureList, int playerCalories,
  ...
  oss << endl << creatureList->toString() << endl;
  return oss.str();
}
...
// ====> DJH: -30% Part A not working
// ====> DJH: -20% Part B not working
// ====> DJH: -20% for timing
// Extension to Sunday was granted and missed. I asked for a note here at the
// bottom of main to remind me. No note was found.

/* ====> DJH: Khaled, this looks like a very late attempt with too little time
  to make substative progress.  My guess is that you understand much more about
  the subject matter than the current score indicates. If you can
  get this in shape by Thursday, there's a chance to improve the score
  substantially. In any event, it will be necessary since the next project
  depends on this code.

  For now, 30/100 in Canvas
*/

// ====> KIA: was granted extention until Sunday 10/22
// ====> DJH: will add ceil(0.8*50) = 40, summing to 70/100
